...
def uploadValues(foldersid):
    user_id = getRowsFromDatabase("SELECT user_id FROM account_emailaddress WHERE email='hackphoenix2021@gmail.com';")[0][0]
    sql_request(f"INSERT INTO user_folderid(user_id,folder_id) VALUES ({user_id}, '{foldersid}');")

def getValues():
    user_id = getRowsFromDatabase("SELECT user_id FROM account_emailaddress WHERE email='hackphoenix2021@gmail.com';")[0][0]

    check = getRowsFromDatabase(f"SELECT folder_id FROM user_folderid WHERE user_id={user_id};")[0][0] #check if this is true/false
